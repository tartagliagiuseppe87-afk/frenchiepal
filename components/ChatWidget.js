...
export default function ChatWidget() {
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content:
        "Ciao, come aiutare il tuo Frenchie oggi?",
    },
  ]);
  ...
  return (
    // H-FULL e W-FULL sono essenziali qui
    <div className="flex flex-col w-full h-full bg-white">
      
      {/* AREA MESSAGGI */}
      <div className="flex-grow p-4 overflow-y-auto flex flex-col gap-4 bg-white scroll-smooth">
        {messages.map((m, i) => (
          <div 
            key={i} 
